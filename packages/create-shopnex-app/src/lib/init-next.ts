...
export async function initNext(args: InitNextArgs): Promise<InitNextResult> {
    ...
    const nextAppDetails =
        args.nextAppDetails || (await getNextAppDetails(projectDir));

    if (!nextAppDetails.nextAppDir) {
        warning(`Could not find app directory in ${projectDir}, creating...`);
        const createdAppDir = path.resolve(
            projectDir,
            nextAppDetails.isSrcDir ? "src/app" : "app"
        );
        fse.mkdirSync(createdAppDir, { recursive: true });
        nextAppDetails.nextAppDir = createdAppDir;
    }

    const { hasTopLevelLayout, isSrcDir, nextAppDir, nextConfigType } =
        nextAppDetails;
...
> {
    ...
    logDebug(
        `Copying template files from ${templateFilesPath} to ${nextAppDir}`
    );

    const templateSrcDir = path.resolve(
        templateFilesPath,
        isSrcDir ? "" : "src"
    );
    ...
    logDebug(
        `payloadConfigPath: ${path.resolve(projectDir, "payload.config.ts")}`
    );
...
export async function getNextAppDetails(
    projectDir: string
): Promise<NextAppDetails> {
    const isSrcDir = fs.existsSync(path.resolve(projectDir, "src"));

    // Match next.config.js, next.config.ts, next.config.mjs, next.config.cjs
    const nextConfigPath: string | undefined = (
        await globby("next.config.(\\w)?(t|j)s", {
            absolute: true,
            cwd: projectDir,
        })
    )?.[0];
    ...
    const packageObj = await fse.readJson(
    ...
    if (packageObj.dependencies?.next) {
        ...
        if (!versionMatch) {
            p.log.warn(
                `Could not determine Next.js version from ${nextVersion}`
            );
            return {
                hasTopLevelLayout: false,
                isSrcDir,
