...
import { linkFieldsSchemaPath } from "./elements/link/shared";
import { uploadFieldsSchemaPath } from "./elements/upload/shared";
...
> = ({
    ...
    const componentMap: Map<string, ClientField[] | React.ReactNode> =
        new Map();
    ...
    (args?.admin?.elements || Object.values(elementTypes)).forEach((el) => {
        ...
        if (element) {
            ...
            switch (element.name) {
                ...
                case "upload": {
                    const uploadEnabledCollections =
                        payload.config.collections.filter(
                            ({
                                admin: { enableRichTextRelationship, hidden },
                                upload,
                            }) => {
                                if (hidden === true) {
                                    return false;
                    ...
                    uploadEnabledCollections.forEach((collection) => {
                        if (
                            args?.admin?.upload?.collections[collection.slug]
                                ?.fields
                        ) {
                            const clientFields = createClientFields({
                                defaultIDType: payload.config.db.defaultIDType,
                                fields: args?.admin?.upload?.collections[
                                    collection.slug
                                ]?.fields,
                                i18n,
                                importMap: payload.importMap,
                            ...
                            componentMap.set(
                                `${uploadFieldsSchemaPath}.${collection.slug}`,
                                clientFields
                            );
                        }
                    });
